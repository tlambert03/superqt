...
if TYPE_CHECKING:
    from superqt.stack_viewer._protocols import PCanvas


def get_canvas(backend: str | None = None) -> type[PCanvas]:
    backend = backend or os.getenv("SUPERQT_CANVAS_BACKEND", None)
    ...
    raise RuntimeError("No canvas backend found. Please install vispy or pygfx.")
